...
class Board {
    ...
    createGrid() {
        ...
        for (var i = 0; i < this.numberOfTiles; i++) {
            ...
            if (this.bottomLayer[i] !== 0) {
                ...
                // collectable.anchor.set(0.5, 0.5);
        ...
        if (amount > 0) {
            ...
            this.bottomTiles.forEach(function(bt) {
                ...
                if (t1 === null && t2 === null && bt.isDead !== true) {
                    ...
                    Tweener.wobbleScaleIn(sprite, 0, 1, .2, function(sprite) {
    ...
    handleMatches(tiles) {
        ...
        for (var i = 0; i < tiles.length; i++) {
            ...
            if ((Settings.interactionType === 'swipe' && (matchComps.h >= 3 || matchComps.v >= 3 || blastMatches.length > 0)) ||
                ...
                //check whether the other matched tiles are boosters

                for(var m = 0; m < matches.length; m++) {

                    var match = matches[m].settings;

                    if(match.key.indexOf('booster') === 0) {

                        if(match.x == tiles[i].settings.x && match.y == tiles[i].settings.y){
                        ...
                        blastMatches = blastMatches.concat(this.getBlastMatches(matches[m]));
    ...
    onGameComplete() {
        ...
        if (Settings.fadeBoardOnComplete === true) {
            ...
            Tweener.fade(this.bgLayerGrp, 0, 0, 2000, Phaser.Easing.Linear.None, true);
            Tweener.fade(this.bottomLayerGrp, 0, 0, 2000, Phaser.Easing.Linear.None, true);
            Tweener.fade(this.belowLayerGrp, 0, 0, 2000, Phaser.Easing.Linear.None, true);
            Tweener.fade(this.aboveLayerGrp, 0, 0, 2000, Phaser.Easing.Linear.None, true);
            Tweener.fade(this.overlayLayerGrp, 0, 0, 2000, Phaser.Easing.Linear.None, true);
